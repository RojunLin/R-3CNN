...
void EltwiseLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.eltwise_param().operation() == 
    EltwiseParameter_EltwiseOp_SUBTRACT) {
    CHECK(top.size() == 1); // only 1 top blob
    CHECK(bottom.size() == 2); //only 2 bottom blobs for operation: bottom[0]-bottom[1]
  }
...
void EltwiseLayer<Dtype>::Forward_cpu(
  ...
  case EltwiseParameter_EltwiseOp_SUM:
    ...
    break;
  case EltwiseParameter_EltwiseOp_SUBTRACT:
  //
  caffe_copy(count, bottom[0]->cpu_data(), top_data);
  caffe_axpy(count, Dtype(-1.), bottom[1]->cpu_data(), top_data);
  break;
    ...
    if (propagate_down[i]) {
      ...
      case EltwiseParameter_EltwiseOp_SUBTRACT:
        if (i == 0) {
          ...
          LOG(ERROR) << "the input blobs must be 2";
        }
        break;
      case EltwiseParameter_EltwiseOp_MAX:
