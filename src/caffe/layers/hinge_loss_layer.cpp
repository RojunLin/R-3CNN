...
void HingeLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int dim = count / num;
  Dtype margin = this->layer_param_.hinge_loss_param().margin();
  caffe_copy(count, bottom_data, bottom_diff);


  // for (int i = 0; i < num; ++i) {
  //   if (static_cast<int>(label[i]) != -1) {
  ...
  //     //   Dtype(0.), 1 + bottom_diff[i * dim + j]);
  //     bottom_diff[i * dim + j] = std::max(
  //       Dtype(0.0), margin + bottom_diff[i * dim + j]);
  //   }
  // }
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < dim; ++j) {
      // LOG(INFO) << label[i] << " " << bottom_diff[i * dim + j];
      bottom_diff[i * dim + j] = std::max(
        Dtype(0.0), margin - label[i] * bottom_diff[i * dim + j]);
    }
  }
